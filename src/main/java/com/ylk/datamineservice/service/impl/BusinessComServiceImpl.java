...
public class BusinessComServiceImpl implements BusinessComService {
	...
	public ResOfflineDataMsg offLineDataUpload(ReqOfflineDataMsg msg) {
		...
		ChargeLog startChargeLog = chargeLogService.getStartChargeByCdzAndCardNo(Integer.valueOf(msg.getCdzNo()), msg.getStrGunType(),
				msg.getCardNo());
		if (startChargeLog == null) {
			logger.info("离线数据上传,无开始充电信息,冻结卡,等待后续处理,cdzNo:{},gunType:{}, cardNo:{}", msg.getCdzNo(), msg.getStrGunType(), msg.getCardNo());
		...
		if (consumFen > cardInfo.getMoney()) {
			// 金额不足，锁定卡
			logger.info("离线数据上传,金额不足情况下，锁定卡,cardNo:{},gunType:{}, 消耗电量:{},单价:{},卡余额:{}", msg.getCardNo(), msg.getStrGunType(), qua,
					areaInfo.getElectricityprice(), cardInfo.getMoney());
		...
		sInfo.setGuntype(msg.getStrGunType());
	...
	public ResPauseChargeMsg pauseCharge(ReqPauseChargeMsg msg) {
		...
		int qua = msg.getMeterQua() - startChargeLog.getBeginMeterQua();
		if (qua < 0) {
			logger.info("充电暂停,消耗电量为负，锁定卡,cardNo:{},开始电量:{},结束电量:{}", msg.getCardNo(), startChargeLog.getBeginMeterQua(),
					msg.getMeterQua());
		}
		int consumFen = (int) Math.ceil(areaInfo.getElectricityprice() * qua / 100d);
		if (consumFen > cardInfo.getMoney()) {
			// 金额不足，锁定卡
			logger.info("充电结束,金额不足情况下，锁定卡,cardNo:{},消耗电量:{},单价:{},卡余额:{}", msg.getCardNo(), qua,
					areaInfo.getElectricityprice(), cardInfo.getMoney());
			cardInfo.setMoney(cardInfo.getMoney().intValue() - consumFen);
			cardInfo.setState(CardStateUtil.CARD_FROZEN);
		} else {
	...
	public ResEndChargeMsg endCharge(ReqEndChargeMsg msg) {
		...
		ChargeLog startChargeLog = chargeLogService.getStartChargeByCdzAndCardNo(Integer.valueOf(msg.getCdzNo()),
				msg.getStrGunType(), msg.getCardNo());
		if (startChargeLog == null) {
			logger.info("充电结束,开始记录错误,冻结卡,等待后续处理,cdzNo:{},gunType:{},cardNo:{}", msg.getCdzNo(), msg.getStrGunType(),
					msg.getCardNo());
			cardInfo.setState(CardStateUtil.DB_CARD_STATE_FROZEN);
			cardInfoService.updateCardInfo(cardInfo);
			return retMsg;
		...
		int qua = msg.getMeterQua() - startChargeLog.getBeginMeterQua();
		if (qua < 0) {
			logger.info("充电结束,消耗电量为负，锁定卡,cardNo:{},开始电量:{},结束电量:{}", msg.getCardNo(), startChargeLog.getBeginMeterQua(),
					msg.getMeterQua());
		}
		// 电表消耗，直接保留
		int consumFen = (int) Math.ceil(areaInfo.getElectricityprice() * qua / 100d);
		if (consumFen > cardInfo.getMoney()) {
			// 金额不足，锁定卡
			logger.info("充电结束,金额不足情况下，锁定卡,cardNo:{},gunType:{},消耗电量:{},单价:{},卡余额:{}", msg.getCardNo(),
					msg.getStrGunType(), qua, areaInfo.getElectricityprice(), cardInfo.getMoney());
			cardInfo.setMoney(cardInfo.getMoney().intValue() - consumFen);
			cardInfo.setState(CardStateUtil.DB_CARD_STATE_FROZEN);
		} else {
			logger.info("充电结束,cardNo:{},gunType:{}, 消耗电量:{},单价:{},卡余额:{}", msg.getCardNo(), msg.getStrGunType(), qua,
					areaInfo.getElectricityprice(), cardInfo.getMoney());
			cardInfo.setMoney(cardInfo.getMoney().intValue() - consumFen);
		...
		try {
			...
			if (msg.getChargeConfig() == ChargeTypeUtil.FIX_DEGREE) {
				...
				if (amount > cardInfo.getMoney().intValue()) {
					logger.info("充电开始固定电度充电,卡号:{},充电电度:{},余额:{},当前单价:{} 余额不足!", cardInfo.getCardno(), degree,
							cardInfo.getMoney(), areaInfo.getElectricityprice());
					retMsg.setChargeCode(ChargeUtil.CHARGE_FORBID_NOT_FUNDS);
				} else {
					logger.info("充电开始固定电度充电,卡号:{},充电电度:{},余额:{},当前单价:{} 开始充电", cardInfo.getCardno(), degree,
							cardInfo.getMoney(), areaInfo.getElectricityprice());
			...
			if (msg.getChargeConfig() == ChargeTypeUtil.FIX_FULL) {
				...
				if (cardInfo.getMoney() < ConstUtil.LEAST_BALANCE) {
					logger.info("充电开始直接充满充电,卡号:{},余额:{},账户余额:{} 小于10元!", cardInfo.getCardno(), cardInfo.getMoney(),
							cardInfo.getMoney());
					retMsg.setChargeCode(ChargeUtil.CHARGE_FORBID_NOT_FUNDS);
				} else {
					logger.info("充电开始直接充满充电,卡号:{},余额:{},账户余额:{} 开始充电!", cardInfo.getCardno(), cardInfo.getMoney(),
							cardInfo.getMoney());
					retMsg.setChargeCode(ChargeUtil.CHARGE_ALLOW);
				}
			}
	...
	public void keepAlive(ResKeepAliveMsg msg) {
		...
		if (msg.getGunType() == 0x0A) {
			if (workState == CdzStateUtil.DEVICE_WORK_WAIT) {
				...
				info.setAgunstate(CdzStateUtil.CDZ_WORK_CONFIG);
			} else if (workState == CdzStateUtil.DEVICE_WORK_CHARGE) {
				info.setAgunstate(CdzStateUtil.CDZ_WORK_CHARGE);
			} else if (workState == CdzStateUtil.DEVICE_WORK_OVER) {
				info.setAgunstate(CdzStateUtil.CDZ_WORK_OVER);
			} else if (workState == CdzStateUtil.DEVICE_WORK_PAUSE) {
				info.setAgunstate(CdzStateUtil.CDZ_WORK_PAUSE);
			} else if (workState == CdzStateUtil.DEVICE_WORK_UPLOAD) {
				info.setAgunstate(CdzStateUtil.CDZ_WORK_UPLOAD);
			} else {
				info.setAgunstate(CdzStateUtil.CDZ_OFFLINE);
			}
		} else {
			if (workState == CdzStateUtil.DEVICE_WORK_WAIT) {
				info.setBgunstate(CdzStateUtil.CDZ_WORK_WAIT);
			} else if (workState == CdzStateUtil.DEVICE_WORK_CONFIG) {
				info.setBgunstate(CdzStateUtil.CDZ_WORK_CONFIG);
			} else if (workState == CdzStateUtil.DEVICE_WORK_CHARGE) {
				info.setBgunstate(CdzStateUtil.CDZ_WORK_CHARGE);
			} else if (workState == CdzStateUtil.DEVICE_WORK_OVER) {
				info.setBgunstate(CdzStateUtil.CDZ_WORK_OVER);
			} else if (workState == CdzStateUtil.DEVICE_WORK_PAUSE) {
				info.setBgunstate(CdzStateUtil.CDZ_WORK_PAUSE);
			} else if (workState == CdzStateUtil.DEVICE_WORK_UPLOAD) {
				info.setBgunstate(CdzStateUtil.CDZ_WORK_UPLOAD);
			} else {
				info.setBgunstate(CdzStateUtil.CDZ_OFFLINE);
			}
		}
